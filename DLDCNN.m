function im_h = DLDCNN(model, im_b)
...
[conv4_channels,conv4_patchsize2] = size(weights_conv4);
...
%% conv4
conv4_data = zeros(hei, wid);
for i = 1 : conv4_channels
    conv4_subfilter = reshape(weights_conv4(i,:), conv4_patchsize, conv4_patchsize);
    conv4_data(:,:) = conv4_data(:,:) + imfilter(conv3_data(:,:,i), conv4_subfilter, 'same', 'replicate');
end

%% Denoising reconstruction
im_h = conv4_data(:,:) + biases_conv4;
